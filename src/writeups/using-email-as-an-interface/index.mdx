...
metaDescription: "Building img-to-pdf, a SaaS with no website: users email images and get a merged PDF back, using Cloudflare Email Routing and Workers as the entire interface."
publishedAt: "2026-05-31"
tags: ["Cloudflare", "Emails"]
readingTime: 16
---

import { ConfirmationHandshake } from "./diagrams/confirmation-handshake.tsx";
import { EmailPipeline } from "./diagrams/pipeline.tsx";
import { MailLoop } from "./diagrams/mail-loop.tsx";
...
In a product with an email interface, the sender's email address is the account identifier (or one of the account identifiers).
...
3. The `Reply-To` header, which tells clients where to send replies.
...
<Note>
  For `img-to-pdf`, I opted to use a limited free tier alongside an unlimited premium tier — whether a sender can perform the merge depends on whether they've exceeded their tier's limits.
...
In the previous step, we parsed the MIME body; now we’ll decode and validate the attachments.
...
After parsing the MIME body, each attachment needs to be extracted as a byte array for inspection before we can process it.
...
Before images are added to the PDF, the system determines the EXIF orientation and applies it when placing the image on a page. Then, using the oriented dimensions, it decides the page's orientation.
...
Cloudflare Email Routing rejects messages larger than 25 MiB before they reach the Worker; however, this does not eliminate the risk of processing.
...
However, PDF generation can still fail because a file may pass inspection but fail during embedding or decoding.
...
If the sender has exceeded the limit, the system replies with a message indicating the limit and when it resets, and includes instructions to reply with `BILLING` to request unlimited conversions.
...
5. The system checks that the reply references the confirmation email.
...
Instead, the service should inspect the file's contents and apply restrictions before processing it.
...
An email address works well as a lightweight account key, but it is not equivalent to an authenticated session.
